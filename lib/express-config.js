...
module.exports.defaultOptions = {
	errorPage: {
		...
		errorLoggingFilter: /** @type {object} */ error => {
	...
	pinoHttp: {
		autoLogging: {
			ignore: request => {
				if (request.url) {
					return new URL(request.url, 'http://example.org').pathname === '/favicon.ico';
				}
				return false;
			}
		},
 ...
 * @property {express.Handler[]} preRoute
 *     Middleware functions to be added before main application routes.
 * @property {express.Handler[]} postRoute
 ...
 * @property {boolean | string | number | string[] | Function} [trustProxy=true]
 ...
 * @property {string | string[]} [viewPath = `${process.cwd()}/views`]
