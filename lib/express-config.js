/**
 * @rowanmanning/express-config module
...
module.exports = function configureExpress(app, options) {
	...
	app.engine('jsx', expressPreactViews.createEngine(options.expressPreact));

	// Augment the app with a logger
	app.log = pino(options.pino);
	...
	app.start = async function start(...args) {
		await listen(...args);
	...
	if (options.redirectToHttps !== false) {
	...
	if (options.pinoHttp !== false) {
		const httpOptions = Object.assign({}, options.pinoHttp, {logger: app.log});
		app.preRoute.push(
 ...
 * @property {(pinoHttp.Options|false)} [pinoHttp]
 ...
 * @property {Array<(String|RegExp)>} [ignoreHosts = [/^localhost:\d+$/i]]
 *     Hostnames on which to not enable the redirect. Ports must be added to manually.
 * @property {Array<(String|RegExp)>} [ignoreRoutes]
